...
This is the ongoing development repository for the Streamr Native SDK, a native counterpart of the [streamr-dev/network](https://github.com/streamr-dev/network).
...
The library can used either directly through its [C API](packages/streamr-libstreamrproxyclient#api-documentation) or through its wrappers. We provide wrappers for [C++](packages/streamr-libstreamrproxyclient/wrappers/cpp), [Go](https://github.com/streamr-dev/goproxyclient), [Python](packages/streamr-libstreamrproxyclient/wrappers/python), [Swift (iOS)](packages/streamr-libstreamrproxyclient/dist/ios-swift-package/StreamrProxyClient), and [Kotlin (Android)](packages/streamr-libstreamrproxyclient/dist/android-library-module/StreamrProxyClient). 

Best way of getting started is to study the example apps for [C](packages/streamr-libstreamrproxyclient/examples/unix-c), [C++](packages/streamr-libstreamrproxyclient/examples/unix-cpp), [Go](packages/streamr-libstreamrproxyclient/examples/go), [Python](packages/streamr-libstreamrproxyclient/examples/python), [Swift (iOS)](packages/streamr-libstreamrproxyclient/examples/ios/LocationShare), and [Kotlin (Android)](packages/streamr-libstreamrproxyclient/examples/android/LocationShare).
...
| File       | Description                                                                 |
|--------------|-----------------------------------------------------------------------------|
| `.vcpkg.json` | Vcpkg dependencies of the whole monorepo. **The `dependencies` array is automatically generated by the `install.sh` script and should not be edited manually.** The `builtin-baseline` key pins the vcpkg ports snapshot; to bump it: update the `vcpkg` submodule to the desired release tag, set `builtin-baseline` to that tag's commit SHA, rebuild everything on all platforms, and revisit every directory under `overlayports/` (each documents in its JUSTIFICATION.md why it exists — delete overlays the new baseline obsoletes). |
