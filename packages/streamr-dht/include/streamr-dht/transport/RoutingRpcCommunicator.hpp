        ...
        this->setOutgoingMessageCallback(
            [this](
                ...
                if (callContext.targetDescriptor.has_value()) {
                    ...
                    SLogger::debug("Set targetDescriptor from callContext.targetDescriptor");
                } else { // rpc reply message
                    SLogger::debug("callContext.targetDescriptor does not have value");
                    if (callContext.incomingSourceDescriptor.has_value()) {
                        SLogger::debug("incomingSourceDescriptor has value");
                    } else {
                        SLogger::debug("incomingSourceDescriptor does not have value");
                    }

                    targetDescriptor =
                        callContext.incomingSourceDescriptor.value();
                    SLogger::debug("Set targetDescriptor from callContext.incomingSourceDescriptor");
                }
