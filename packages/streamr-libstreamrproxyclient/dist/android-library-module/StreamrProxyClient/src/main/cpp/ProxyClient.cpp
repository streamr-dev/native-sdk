...
jobject createStreamrError(JNIEnv* env, const Error& error) {
    ...
    if (strcmp(error.code, ERROR_INVALID_ETHEREUM_ADDRESS) == 0) {
        ...
        errorClassName =
            "network/streamr/proxyclient/StreamrError$ProxyBroadcastFailed";
    } else {
        errorClassName =
    ...
    jmethodID constructor =
        env->GetMethodID(errorClass, "<init>", "(Ljava/lang/String;)V");
...
void throwProxyClientException(JNIEnv* env, jobject streamrError) {
    ...
    jobject exception =
        env->NewObject(exceptionClass, constructor, streamrError);
...
jobject createProxyAddress(JNIEnv* env, const Proxy& proxy) {
    ...
    if (proxyAddressClass == nullptr) {
        return nullptr;
    }

    jmethodID constructor = env->GetMethodID(
        proxyAddressClass, "<init>", "(Ljava/lang/String;Ljava/lang/String;)V");
    ...
    jobject proxyAddress = env->NewObject(
        proxyAddressClass, constructor, websocketUrl, ethereumAddress);
...
jobject createProxyResult(
    JNIEnv* env,
    uint64_t numConnected,
    ...
    for (const auto& proxy : successful) { // NOLINT
    ...
    for (const auto& error : errors) { // NOLINT
        ...
        jobject proxyError = env->NewObject(
            proxyErrorClass, proxyErrorConstructor, streamrError, proxyAddress);
...
Java_network_streamr_proxyclient_ProxyClientJNI_proxyClientNew( // NOLINT
    JNIEnv* env,
    jclass /* clazz */,
    jstring jOwnEthereumAddress,
    jstring jStreamPartId) {
    const char* ownEthereumAddress =
        env->GetStringUTFChars(jOwnEthereumAddress, nullptr);
    const char* streamPartId = env->GetStringUTFChars(jStreamPartId, nullptr);
...
Java_network_streamr_proxyclient_ProxyClientJNI_proxyClientDelete( // NOLINT
...
Java_network_streamr_proxyclient_ProxyClientJNI_proxyClientConnect( // NOLINT
    JNIEnv* env, jobject /* thiz */, jlong handle, jobject jProxies) {
    // Get List size and method
    jclass listClass = env->GetObjectClass(jProxies);
    jint numProxies = env->CallIntMethod(
        jProxies, env->GetMethodID(listClass, "size", "()I"));
    jmethodID getMethod =
        env->GetMethodID(listClass, "get", "(I)Ljava/lang/Object;");

    // Create C proxy array
    std::vector<Proxy> proxies(numProxies); // NOLINT
    std::vector<std::string> websocketUrls(numProxies); // NOLINT
    std::vector<std::string> ethereumAddresses(numProxies); // NOLINT
    ...
    for (jint i = 0; i < numProxies; i++) {
        ...
        auto jWebsocketUrl = static_cast<jstring>(
            env->CallObjectMethod(jProxy, getWebsocketUrl));
        auto jEthereumAddress = static_cast<jstring>(
            env->CallObjectMethod(jProxy, getEthereumAddress));
