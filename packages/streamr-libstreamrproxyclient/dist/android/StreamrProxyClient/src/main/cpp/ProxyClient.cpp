...
Java_network_streamr_proxyclient_ProxyClientJNI_proxyClientNew(JNIEnv *env, jclass, jstring jOwnEthereumAddress, jstring jStreamPartId) {
    ...
    if (proxyResult && proxyResult->numErrors > 0) {
     ...
     // Get the constructor
        ...
        jstring proxyUrl = error.proxy ? env->NewStringUTF(error.proxy->websocketUrl) : nullptr;
        jstring proxyAddress = error.proxy ? env->NewStringUTF(error.proxy->ethereumAddress) : nullptr;

        // Create and throw the exception
        jobject exception = env->NewObject(exceptionClass, constructor, 
            message, code, proxyUrl, proxyAddress);
...
Java_network_streamr_proxyclient_ProxyClientJNI_proxyClientConnect(
    ...
    jmethodID getWebsocketUrl = env->GetMethodID(proxyAddressClass, "getWebsocketUrl", "()Ljava/lang/String;");
    jmethodID getEthereumAddress = env->GetMethodID(proxyAddressClass, "getEthereumAddress", "()Ljava/lang/String;");
    ...
    if (proxyResult != nullptr) {
        ...
        jmethodID constructor = env->GetMethodID(proxyResultClass, "<init>", 
        ...
        for (uint64_t i = 0; i < proxyResult->numSuccessful; i++) {
            ...
            jobject proxyAddress = env->NewObject(proxyAddressClass, 
                env->GetMethodID(proxyAddressClass, "<init>", "(Ljava/lang/String;Ljava/lang/String;)V"),
        ...
        jclass streamrErrorClass = env->FindClass("network/streamr/proxyclient/StreamrError");
        ...
        for (uint64_t i = 0; i < proxyResult->numErrors; i++) {
            ...
            if (strcmp(error.code, ERROR_INVALID_PROXY_URL) == 0) {
                jclass errorClass = env->FindClass("network/streamr/proxyclient/StreamrError$InvalidProxyUrl");
                jmethodID errorConstructor = env->GetMethodID(errorClass, "<init>", "(Ljava/lang/String;)V");
                streamrError = env->NewObject(errorClass, errorConstructor, message);
                ...
                jmethodID errorConstructor = env->GetMethodID(errorClass, "<init>","(Ljava/lang/String;)V");
                streamrError = env->NewObject(errorClass, errorConstructor, message);
            } else if (strcmp(error.code, ERROR_INVALID_ETHEREUM_ADDRESS) == 0) {
                jclass errorClass = env->FindClass("network/streamr/proxyclient/StreamrError$InvalidEthereumAddress");
                jmethodID errorConstructor = env->GetMethodID(errorClass, "<init>", "(Ljava/lang/String;)V");
                streamrError = env->NewObject(errorClass, errorConstructor, message);
            } else {
                jclass errorClass = env->FindClass("network/streamr/proxyclient/StreamrError$UnknownError");
                jmethodID errorConstructor = env->GetMethodID(errorClass, "<init>", "(Ljava/lang/String;)V");
                streamrError = env->NewObject(errorClass, errorConstructor, message);
            }
            ...
            jstring ethereumAddress = env->NewStringUTF(error.proxy->ethereumAddress);
            jobject errorProxy = env->NewObject(proxyAddressClass,
                env->GetMethodID(proxyAddressClass, "<init>", "(Ljava/lang/String;Ljava/lang/String;)V"),
                websocketUrl, ethereumAddress);
                
            // Create ProxyError
            jobject proxyError = env->NewObject(proxyErrorClass,
                env->GetMethodID(proxyErrorClass, "<init>", 
                    "(Lnetwork/streamr/proxyclient/StreamrError;Lnetwork/streamr/proxyclient/StreamrProxyAddress;)V"),
                streamrError, errorProxy);
        ...
        result = env->NewObject(proxyResultClass, constructor, 
            static_cast<jlong>(numConnected), successfulList, failedList);
...
Java_network_streamr_proxyclient_ProxyClientJNI_proxyClientPublish(
        JNIEnv* env, jobject /* this */, jlong handle, jstring jContent, jstring jEthereumPrivateKey) {
    ...
    if (proxyResult != nullptr) {
        ...
        if (proxyResult->numSuccessful > 0) {
            jclass proxyAddressClass = env->FindClass("network/streamr/proxyclient/StreamrProxyAddress");
            jmethodID proxyAddressConstructor = env->GetMethodID(proxyAddressClass, "<init>",
            ...
            for (uint64_t i = 0; i < proxyResult->numSuccessful; i++) {
                ...
                jobject proxyAddress = env->NewObject(proxyAddressClass, proxyAddressConstructor,
                                                      websocketUrl, ethereumAddress);
                env->CallBooleanMethod(successfulList, addMethod, proxyAddress);
        ...
        // Create final result
        result = env->NewObject(proxyResultClass, constructor,
                                static_cast<jlong>(numPublished), successfulList, failedList);
