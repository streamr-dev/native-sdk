 ...
 *
 * Supported formats:
 *  - full stream id format, e.g.,
 * '0x0000000000000000000000000000000000000000/foo/bar' or 'name.eth/foo/bar'
 *  - path-only format, e.g. , '/foo/bar'
 *  - legacy format, e.g., '7wa7APtlTq6EC5iTCBy6dw'
 *
 *  If `streamIdOrPath` is not in path-only format, `domain` can be left
 * undefined.
 */
inline StreamID toStreamID(
    std::string_view streamIdOrPath,
    const std::optional<std::variant<EthereumAddress, ENSName>>& domain =
        std::nullopt) {
    ...
    if (firstSlashIdx == 0) { // path-only format
        if (!domain.has_value()) {
            throw std::runtime_error(
                "path-only format \"" + std::string(streamIdOrPath) +
                "\" provided without domain");
        }
        return std::visit(
            [&streamIdOrPath](const auto& value) {
                return StreamID{
                    std::string(value) + std::string(streamIdOrPath)};
            },
            domain.value());
    }
    const auto dom =
        toEthereumAddressOrENSName(streamIdOrPath.substr(0, firstSlashIdx));
...
class StreamIDUtils {
    ...
    static std::optional<std::pair<std::string, std::string>> getDomainAndPath(
